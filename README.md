...
- 來源名稱只保留最後一段路徑（`team/foo` -> `foo`）。沒有 tag（或 tag 為 `latest`）且最後一段含底線的名稱，會以最後一個底線切成名稱與 tag（`team/foo_bar` -> `foo:bar`、`foo_bar_baz` -> `foo_bar:baz`）；有明確 tag 的則原樣保留（`foo_bar:1.2.3` 不變）。名稱中剩下的連字號會統一改成底線（`kd-table-purge_1.0` -> `kd_table_purge:1.0`）；tag 永遠不會被改寫，所以 `foo-bar:v1-rc1` 會變成 `foo_bar:v1-rc1`。其餘沒有 tag 的名稱會補上 `:latest`。Digest 形式（`name@sha256:…`）無法重新打 tag，會被記為失敗。
